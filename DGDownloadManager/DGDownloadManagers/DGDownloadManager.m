...
- (NSString *)getCache:(NSString *)downloadUrl withResponse:(NSURLResponse *)response customCahceName:(NSString *)customCacheName{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *cachePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"DGMusic"];
    ...
    if (customCacheName.length > 0) {
        ...
        cachePath = [cachePath stringByAppendingPathComponent:[[[downloadUrl dataUsingEncoding:NSUTF8StringEncoding] DG_MD5HashString] stringByAppendingString:@".download"]];
    }
    if (![fileManager fileExistsAtPath:cachePath]) {
